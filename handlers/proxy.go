...
func (l *FunctionLookup) byName(ctx context.Context, name string) (string, error) {
	...
	serviceFilter.Add("name", ProjectSpecificName(name))
	...
	if len(services) > 0 {
		return ProjectSpecificName(name), nil
	}
...
func (l *FunctionLookup) byDNSRoundRobin(ctx context.Context, name string) (string, error) {
	entries, lookupErr := l.dnsrrLookup(ctx, fmt.Sprintf("tasks.%s", ProjectSpecificName(name)))
